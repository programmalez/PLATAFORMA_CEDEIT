...
@section('content_header')
    <h1 style="margin-bottom: 20px; text-align: center;">Crear Cursos CETFOC</h1>
@stop
...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('createcurs') }}" method="post" enctype="multipart/form-data">
                @csrf
            ...
            <div class="col-12 mt-3" id="Tabla_Ejecutar_Tareas" style="overflow-x: auto;">
                <div class="table-container">
                    <table class="table table-bordered table-striped" style="width: 100%;">
                        ...
                        <tbody>
                            <?php
                            $procet = \App\Models\ProcetModel::select('id','curso','licencia','perfil','cuatri','imagen','documento','titulo','modalidad','texto')->get();
                            ?>
                            @foreach ($procet as $proce)
                                <tr>
                                    <td> {{ $proce->id  }} </td>
                                    <td> {{ $proce->curso }} </td>
                                    <td> {{ $proce->titulo }}</td>
                                    <td> {{ $proce->modalidad }}</td>
                                    <td> {{ $proce->cuatri}}</td>
                                    <td> {{ substr($proce->perfil, 0, 30) }}{{ strlen($proce->perfil) > 30 ? '...' : '' }}
                                    <td> {{ substr($proce->texto, 0, 30) }}{{ strlen($proce->texto) > 30 ? '...' : '' }} </td>
                                    <td> {{ $proce->documento }}</td>
                                    <td>
                                        <form action="{{ route('edit', ['id' => $proce->id]) }}" method="get" style="display:inline;">
                                            @csrf
                                            <button type="submit" class="btn btn-warning btn-accion" ><i class="fas fa-pen"></i></button>
                                        </form>
                                        <form action="{{ route('delete_cur', ['id' => $proce->id]) }}" method="post" style="display:inline;">
                                            @csrf
                                            @method('DELETE')

...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('createcurs2') }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="ocupacional" name="ocupacional" style="width: 100%; height: 200px;"></textarea>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
            ...
            <div class="col-12 mt-3" id="Tabla_Ejecutar_Tareas" style="overflow-x: auto;">
                <div class="table-container">
                    <table class="table table-bordered table-striped" style="width: 100%;">
                        ...
                        <tbody>
                            <?php
                            $progmagdalena = \App\Models\PromagdalenaModel::select('id','programa','codigo','metodologia','texto','semestre','documento')->get();
                            ?>
                            @foreach ($progmagdalena as $progmag)
                                <tr>
                                    <td> {{ $progmag->id  }} </td>
                                    <td> {{ $progmag->programa }} </td>
                                    <td> {{ $progmag->codigo }} </td>
                                    <td> {{ $progmag->metodologia }} </td>
                                    <td> {{ substr($progmag->texto, 0, 30) }}{{ strlen($progmag->texto) > 30 ? '...' : '' }}
                                    <td> {{ $progmag->semestre}}</td>
                                    <td> {{ $progmag->documento }}</td>
                                    <td>
                                        <form action="{{ route('edit2', ['id' => $progmag->id]) }}" method="get" style="display:inline;">
                                            @csrf
                                            <button type="submit" class="btn btn-warning btn-accion" ><i class="fas fa-pen"></i></button>
                                        </form>
                                        <form action="{{ route('delete2', ['id' => $progmag->id]) }}" method="post" style="display:inline;">
                                            @csrf
                                            @method('DELETE')

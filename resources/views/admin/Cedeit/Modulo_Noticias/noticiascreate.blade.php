...
@section('content_header')
    <h1 style="margin-bottom: 20px; text-align: center;">Agregar Noticias Cedeit</h1>
@stop
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="time" class="form-control" id="hora_publicacion"
                ...
                <br>
                <div class="col-12 mt-3" id="Tabla_Noticias_Cedeit" style="overflow-x: auto;">
                    <div class="table-container">
                        <table class="table table-bordered table-striped" style="width: 100%;">
                            ...
                            <tbody>
                                <?php
                                $noticias = \App\Models\NoticiascedeitModel::select('id', 'nombre', 'imagen', 'fecha_publicacion', 'hora_publicacion', 'texto', 'autor', 'enlace')->get();
                                ?>
                                @foreach ($noticias as $noticia)
                                    <tr>
                                        <td> {{ $noticia->id }} </td>
                                        <td> {{ $noticia->nombre }} </td>
                                        <td> {{ substr($noticia->texto, 0, 30) }}{{ strlen($noticia->texto) > 30 ? '...' : '' }}
                                        </td>
                                        <td> {{ $noticia->autor }}</td>
                                        <td> {{ $noticia->fecha_publicacion }}</td>
                                        <td> {{ $noticia->hora_publicacion }}</td>
                                        <td>
                                            <form action="{{ route('edit_noticia', ['id' => $noticia->id]) }}"
                                                method="get" style="display:inline;">
                                                @csrf
                                                <button type="submit" class="btn btn-warning btn-accion"><i
                                                        class="fas fa-pen"></i></button>
                                            </form>
                                            <form action="{{ route('delete_noti', ['id' => $noticia->id]) }}" method="post"
...
@section('css')
    <style>
        ...
        .btn-accion {
            padding: 5px 7px;
            /* Ajusta el padding según tus preferencias para cambiar el tamaño */
            font-size: 12px;

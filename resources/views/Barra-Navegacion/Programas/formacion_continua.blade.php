...
.post-img-1{
    background: url('/assets/seminario.png');
    width: 490px;
    height: 390px;
...
<body>
    ...
    <section id="seccion9">
        <h2 style="color:  #094d2c; font-size: 24px; text-align: left; text-align: center;">Centro de Estudios Tecnicos y Formación Continua</h2>
    </section>
    <section class="post-list">
        <div class="content">
            <?php
            $procedeit = \App\Models\ProcedModel::select('id','curso','titulo', 'imagen')->get();
            ?>
            @foreach($procedeit as $proced)
                <article class="post">
                    <div class="post-header">
                        <div class="post-img-1"><img src="{{ asset('Imagenes/'.$proced->imagen) }}" class="card-img-top img-fluid"  alt="Imagen" style="width: 490px; height: 390px;"></div>
                    </div>
                    <div class="post-body">
                        <span>{{ $proced->titulo }}</span>
                        <h2>{{ $proced->curso }}</h2>
                        <a href="Rcedeit" class="post-link">Inscribete Ahora</a>
                    </div>
                </article>
            @endforeach
        </div>
